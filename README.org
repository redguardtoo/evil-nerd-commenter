...
All you need to is =M-x evilnc-toggle-invert-comment-line-by-line= then =C-u 2 evilnc-comment-or-uncomment-lines=. The first command turn on some flag, so the behaviour of (un)commenting is different.
* Evil usage
...
You can assign other key instead of "c" to the text object by customising =evilnc-comment-text-object=.
